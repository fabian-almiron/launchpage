  ...
  sidebarElement.innerHTML = `
    ...
    <div class="mt-6 h-0 flex-1 flex flex-col overflow-y-auto">
      <nav class="flex-1 px-2 space-y-1">
        ${menuItems.map(item => `
          <a href="${item.href}" class="${item.id === currentPage ? 'nav-link-active' : 'nav-link'}">
            ${item.icon}
            ${item.label}
          </a>
        `).join('')}
      ...
      <div class="px-2 space-y-1 pt-6 pb-2">
        <button onclick="logout()" class="nav-link text-red-700 hover:bg-red-50 hover:text-red-700">
